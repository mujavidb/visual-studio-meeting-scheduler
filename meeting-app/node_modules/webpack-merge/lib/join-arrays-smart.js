...
function uniteRules(newRule, rule, strategy) {
  ...
  if (rule.rules) {
    var _uniteRules = function _uniteRules(b, a) {
      return uniteRules(b, a, strategy);
    };
    switch (strategy) {
      case 'prepend':
        rule.rules = [].concat(_toConsumableArray((0, _differenceWith3.default)(newRule.rules, rule.rules, _uniteRules)), _toConsumableArray(rule.rules));
        break;
      default:
        rule.rules = (0, _unionWith3.default)(rule.rules, newRule.rules, uniteRules);
    }
  }
